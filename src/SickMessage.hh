...
namespace sickpls {
    ...
    public:
        ...
        static const unsigned int MESSAGE_MAX_LENGTH =
                MESSAGE_HEADER_LENGTH + MESSAGE_PAYLOAD_MAX_LENGTH + MESSAGE_TRAILER_LENGTH;

        /** A standard constructor */
        SickMessage();

        /** Construct a well-formed Sick message */
        void BuildMessage(const uint8_t* payload_buffer, unsigned int payload_length);

        /** Populates fields given a sequence of bytes representing a raw message */
        virtual void ParseMessage(const uint8_t* message_buffer) = 0;

        /** Returns a copy of the raw message buffer */
        void GetMessage(uint8_t* message_buffer) const;

        /** Resturns the total message length in bytes */
        [[nodiscard]] unsigned int GetMessageLength() const { return _message_length; }

        /** Returns a copy of the raw message payload */
        void GetPayload(uint8_t* payload_buffer) const;

        /** Returns a copy of the payload as a C String */
        void GetPayloadAsCStr(char* payload_str) const;

        /** Returns a subregion of the payload specified by indices */
        void GetPayloadSubregion(uint8_t* payload_sub_buffer, unsigned int start_idx,
                                 unsigned int stop_idx) const;

        /** Returns the total payload length in bytes */
        [[nodiscard]] unsigned int GetPayloadLength() const { return _payload_length; }

        /** Indicates whether the message container is populated */
        [[nodiscard]] bool IsPopulated() const { return _populated; };
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::SickMessage() = default;
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::BuildMessage(
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::ParseMessage(
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::GetMessage(
            uint8_t* const message_buffer) const {
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::GetPayload(
            uint8_t* const payload_buffer) const {
        memcpy(payload_buffer, &_message_buffer[MESSAGE_HEADER_LENGTH], _payload_length);
    }

    /** Returns a copy of the payload as a C String */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::GetPayloadAsCStr(
            char* const payload_buffer) const {
        memcpy(payload_buffer, &_message_buffer[MESSAGE_HEADER_LENGTH], _payload_length);
     ...
     */
    template<unsigned int MSG_HEADER_LENGTH, unsigned int MSG_PAYLOAD_MAX_LENGTH, unsigned int MSG_TRAILER_LENGTH>
    void SickMessage<MSG_HEADER_LENGTH, MSG_PAYLOAD_MAX_LENGTH, MSG_TRAILER_LENGTH>::GetPayloadSubregion(
            uint8_t* const payload_sub_buffer,
