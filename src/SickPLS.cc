...
namespace sickpls {
     ...
     */
    SickPLS::SickPLS(std::string  sick_device_path) : SickLIDAR<SickPLSBufferMonitor, SickPLSMessage>(),
                                                           _sick_device_path(std::move(sick_device_path)),
     ...
     */
    void
    SickPLS::Uninitialize() noexcept(false) {
     ...
     */
    double SickPLS::GetSickScanAngle() const noexcept(false) {
     ...
     */
    double SickPLS::GetSickScanResolution() const noexcept(false) {
     ...
     */
    sick_pls_measuring_units_t SickPLS::GetSickMeasuringUnits() const noexcept(false) {
     ...
     */
    sick_pls_operating_mode_t SickPLS::GetSickOperatingMode() const noexcept(false) {
    ...
    void SickPLS::GetSickScan(unsigned int* const measurement_values,
                              unsigned int& num_measurement_values) noexcept(false) {
     ...
     */
    void SickPLS::ResetSick() noexcept(false) {
     ...
     */
    void SickPLS::_setupConnection() noexcept(false) {
     ...
     */
    void SickPLS::_teardownConnection() noexcept(false) {
     ...
     */
    void SickPLS::_flushTerminalBuffer() noexcept(false) {
     ...
     */
    void SickPLS::_setSessionBaud(
            const sick_pls_baud_t baud_rate) noexcept(false) {
        ...
        try {
            ...
            _sendMessageAndGetReply(message, response, DEFAULT_SICK_PLS_SICK_MESSAGE_TIMEOUT,
     ...
     */
    bool SickPLS::_testSickBaud(const sick_pls_baud_t baud_rate) noexcept(false) {
     ...
     */
    void SickPLS::_setTerminalBaud(const sick_pls_baud_t baud_rate) noexcept(false) {
        ...
        try {
            ...
            if (baud_rate == SICK_BAUD_500K) {
                ...
                if (ioctl(_sick_fd, TIOCGSERIAL, &serial) < 0) {
                    std::cerr << "SickPLS::_setTermSpeed: ioctl() failed while trying to get serial port info!"
                              << std::endl;
                ...
                if (ioctl(_sick_fd, TIOCSSERIAL, &serial) < 0) {
                    std::cerr << "SickPLS::_setTerminalBaud: ioctl() failed while trying to set serial port info!"
                              << std::endl;
                    std::cerr << "\tNOTE: This is normal when connected via USB!" << std::endl;
                }
    ...
    void SickPLS::_getSickErrors(unsigned int* const num_sick_errors, uint8_t* const error_type_buffer,
                                 uint8_t* const error_num_buffer) noexcept(false) {
     ...
     */
    void SickPLS::_parseSickScanProfileB0(const uint8_t* const src_buffer,
                                          sick_pls_scan_profile_b0_t& sick_scan_profile) const {
     ...
     */
    bool SickPLS::_validSickScanAngle(const sick_pls_scan_angle_t sick_scan_angle) {
     ...
     */
    bool SickPLS::_validSickScanResolution(const sick_pls_scan_resolution_t sick_scan_resolution) {
     ...
     */
    sick_pls_baud_t SickPLS::_baudToSickBaud(const int baud_rate) {
