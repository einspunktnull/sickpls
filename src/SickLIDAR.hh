...
namespace sickpls {
    ...
    protected:
        ...
        void _startListening() noexcept(false);

        /** Stops the driver from listening */
        void _stopListening() noexcept(false);

        /** Indicates whether there is a monitor currently running */
        [[nodiscard]] bool _monitorRunning() const { return _sick_monitor_running; }

        /** Make the associated file descriptor non blocking */
        void _setBlockingIO() const noexcept(false);

        /** Make the associated file descriptor non blocking */
        void _setNonBlockingIO() const noexcept(false);

        /** Send a message to the Sick LD (allows specifying min time between transmitted bytes) */
        void _sendMessage(const SICK_MSG_CLASS& sick_message, unsigned int byte_interval) const
        noexcept(false);

        /** Acquire the next message from the message container */
        void _recvMessage(SICK_MSG_CLASS& sick_message, unsigned int timeout_value) const noexcept(false);

        /** Search the stream for a payload with a particular "header" byte string */
        void _recvMessage(SICK_MSG_CLASS& sick_message,
                          const uint8_t* byte_sequence,
                          unsigned int byte_sequence_length,
                          unsigned int timeout_value) const noexcept(false);

        /** An inline function for computing elapsed time */
        [[nodiscard]] double _computeElapsedTime(const struct timeval& beg_time, const struct timeval& end_time) const {
     ...
     */
    template<class SICK_MONITOR_CLASS, class SICK_MSG_CLASS>
    void SickLIDAR<SICK_MONITOR_CLASS, SICK_MSG_CLASS>::_startListening() noexcept(false) {
     ...
     */
    template<class SICK_MONITOR_CLASS, class SICK_MSG_CLASS>
    void SickLIDAR<SICK_MONITOR_CLASS, SICK_MSG_CLASS>::_stopListening() noexcept(false) {
     ...
     */
    template<class SICK_MONITOR_CLASS, class SICK_MSG_CLASS>
    void SickLIDAR<SICK_MONITOR_CLASS, SICK_MSG_CLASS>::_setBlockingIO() const noexcept(false) {
     ...
     */
    template<class SICK_MONITOR_CLASS, class SICK_MSG_CLASS>
    void SickLIDAR<SICK_MONITOR_CLASS, SICK_MSG_CLASS>::_setNonBlockingIO() const noexcept(false) {
     ...
     */
    template<class SICK_MONITOR_CLASS, class SICK_MSG_CLASS>
    void SickLIDAR<SICK_MONITOR_CLASS, SICK_MSG_CLASS>::_sendMessage(const SICK_MSG_CLASS& sick_message,
                                                                     const unsigned int byte_interval) const
    noexcept(false) {
